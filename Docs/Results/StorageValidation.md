...
2026-09-22 Gitea 저장소를 공개로 전환했다. 브라우저 쿠키·사용자 자격 증명을 전달하지 않은 별도 HTTP 요청으로 LFS batch download를 호출하고 반환된 다운로드 주소에서 샘플 자산을 받았다. `IA_Move.uasset`의 SHA-256이 위 원본 값과 일치했다. 기존 전체 753개 복원 검증과 별개로, 이 검사는 공개 전환 후 익명 다운로드 경로를 샘플 하나로 확인한 것이다.
