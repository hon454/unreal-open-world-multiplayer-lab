...
현재는 PIE 2인 검증까지만 진행한다. 외부 실행 스크립트·패키징된 서버와 실제 클라이언트 2개 접속·EOS 접속·인증 검증은 후속 단계다.
[접속 아키텍처](Architecture/ConnectionFlow.md)와 [현재 검증 범위](Architecture/README.md#현재-범위)를 따른다.
